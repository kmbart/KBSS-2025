#  WklyStatRpt - generate stats on a weekly basis for part or all of a season
...
prevSeasonLastWeek = str(prevSeasonCCYY) + SD.weeks[prevSeasonCCYY][-2]
prevWeek           = ''
...
# Generate dictionary of previous season's positions for all players
    _SQL = 'select * from Weekly_Stats_' + str(seasonCCYY) + ' where CCYYMMDD = ?'
...
#                        print ('player:', prevSeasonLastWeek, player[2], player[3], player[4])

                        if prevEOYStats != None:
                            prevPos = prevPosDict[playerID]
                            print ('pID=', playerID, ',prevPos=', prevPos, ',currPos=', currPos)
                            bothPos = ''
