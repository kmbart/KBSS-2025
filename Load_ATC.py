#  LoadSteamer - get Steamer Projections for Hitters and Pitchers
...
#  set the path for the projections file
PathName = 'C:\\PROJECTIONS\\Projections' + SeasonCCYY + '\\'
print ('path is:', PathName)

#  set the projections source name
ProjectionsSource = 'ATC'
...
#  HITTER TABLE
#  set the projections hitter table name
TableName = 'Projections_' + ProjectionsSource + '_H'
...
#  set the hitter filename
FileName = 'ATC Hitters - ' + SeasonCCYY + '.csv'
print('filename:', FileName)

#  read in the hitter file without the header line
try:
    with open(PathName + FileName, 'r') as HitterFile:
        reader = csv.reader(HitterFile)
...
#  SELECT the projections hitter table rows with NULL RW_ID and show them
    ...
    Cnt_Nulls = 0
    for row in rows:
        if (row[3] != '' and row[3] not in AL_Teams):
            Cnt_Nulls += 1
            print (row)
...
#  set the projections pitcher table name
TableName = 'Projections_' + ProjectionsSource + '_P'
print('proj tbl name:', TableName)
...
#  set the pitcher filename
FileName = 'ATC Pitchers - ' + SeasonCCYY + '.csv'
print('filename:', FileName)
